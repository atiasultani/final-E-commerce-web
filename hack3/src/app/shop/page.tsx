...
const Shop = async () => {
 ...
 const handleAddToCart=(e:React.MouseEvent,product:Product)=>{e.preventDefault()
  Swal.fire({
        ...
        {products.map((product) => (
          <Link key={product._id} href={`/shop/${product.slug}`}>
            ...
            <div className="border rounded-lg p-4 flex flex-col items-center bg-white shadow hover:shadow-lg transition-shadow cursor-pointer">
              ...
              <div>  <button    className="bg-blue-600 text-white px-4 py-3 mt-3 rounded-lg"      
           onClick={ (e) => handleAddToCart(e,product)}> Add To Cart</button></div>               
            </div>
          </Link>
        ))}
