...
const getData = async (): Promise<Product[]> => {
  return await client.fetch(`*[_type == "products"][7...12]{
    _id,
    "slug": slug.current,
    mainHeading,
    name,
  ...
  return (
    <div className="container mx-auto px-6 py-10">
      <h1 className="text-3xl font-extrabold text-center text-gray-800 mb-8">
        {products[0]?.mainHeading || 'New Arrivals Items'}
      </h1>
      <div className=" grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
        {products.map((product) => (
          <Link key={product._id} href={`/newArrivals/${product.slug}`} 
          className="transform transition-transform hover:scale-105">
            <div className="border rounded-xl p-5 flex flex-col items-center bg-white shadow-lg hover:shadow-2xl transition-shadow cursor-pointer w-full max-w-xs h-auto flex-grow">
              ...
              <p className="text-gray-600 mb-1">Price: <span className=" text-red-500">${product.price}</span></p>
